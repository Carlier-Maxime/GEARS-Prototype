...
FColor FGridSettingsCustomization::GenerateColor(const UGridSettings& Ctx, const FGridPosition& Pos) const
{
	const auto Index = Generator.GetValue().SampleBiome(Pos);
	return Index==-1 ? FColor::Black : GridParams::Get().GetBiomeRegistry()[Index].Get()->Color.ToFColorSRGB();
}
